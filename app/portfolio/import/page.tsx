...
export default async function PortfolioImportPage() {
  ...
  const [holdings, template, sharedSidebarRailData] = await Promise.all([
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px] xl:items-start">
          ...
          <aside className="space-y-3 xl:sticky xl:top-24">
            ...
            <GlowCard className="space-y-3">
              <p className="text-sm font-semibold text-[#1B3A6B]">Import checklist</p>
              <ul className="space-y-2 text-sm leading-6 text-[rgba(75,85,99,0.84)]">
                <li>Use the sample template if you want the fastest setup.</li>
                <li>Keep one holding per row.</li>
                <li>Check the file before import so bad rows stay out.</li>
                <li>Use the portfolio page after import to edit or remove positions.</li>
              </ul>
            </GlowCard>
          </aside>
        </div>
