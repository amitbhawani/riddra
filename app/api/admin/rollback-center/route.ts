...
import {
  addRollbackScenario,
  removeRollbackScenario,
  saveRollbackQueueState,
} from "@/lib/editorial-revision-memory-store";
...
export async function POST(request: Request) {
  ...
  if (!asset || !change || !queueState) {
    return NextResponse.json(
      {
        error:
          "Provide asset, change, and queue state for a rollback update, or provide the full rollback scenario payload to create a new scenario.",
      },
      { status: 400 },
    );
  }

  const revisionMemory = await saveRollbackQueueState({
    asset,
    change,
    queueState,
  });
