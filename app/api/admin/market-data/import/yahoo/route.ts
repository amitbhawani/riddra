...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const yahooSymbol = cleanString(payload.yahooSymbol, 80);
    if (!yahooSymbol) {
      return NextResponse.json(
        { error: "Enter a Yahoo Finance symbol such as RELIANCE.NS." },
        { status: 400 },
      );
    }
    ...
    if (mode === "preview") {
      const preview = (await runMarketDataIngestion("yahoo_finance", {
        mode: "preview",
        type: "stock_ohlcv",
        yahooSymbol,
        ...
        fileName:
          cleanString(payload.fileName, 240) ||
          `${yahooSymbol.replace(/[^A-Za-z0-9._-]+/g, "-")}-yahoo-finance.csv`,
      ...
      await appendAdminActivityLog({
        ...
        targetId: null,
        targetFamily: "stock_ohlcv",
        targetSlug: null,
        summary: `Previewed Yahoo Finance stock data for ${yahooSymbol}.`,
        metadata: {
    ...
    const result = (await runMarketDataIngestion("yahoo_finance", {
      mode: "execute",
      type: "stock_ohlcv",
      yahooSymbol,
      executionMode: parseExecutionMode(payload.executionMode),
    ...
    try {
      await appendAdminActivityLog({
        ...
        summary:
          result.batch.status === "failed"
            ? `Yahoo Finance import failed for ${yahooSymbol}.`
            : `Imported ${result.batch.successCount} stock_ohlcv row${result.batch.successCount === 1 ? "" : "s"} from Yahoo Finance for ${yahooSymbol}.`,
        metadata: {
      ...
      const warning = `Activity log warning: ${activityError instanceof Error ? activityError.message : "Could not append Yahoo Finance import activity log after data writes."}`;
    ...
    const message =
      error instanceof Error
        ? error.message
        : "We could not process that Yahoo Finance market-data import right now.";
    ...
    if (actorUserId) {
      try {
        await appendAdminActivityLog({
          ...
          targetFamily: "stock_ohlcv",
          targetSlug: null,
          summary: `Yahoo Finance import failed for ${cleanString(payload.yahooSymbol, 80) || "unknown symbol"}.`,
          metadata: {
            sourceType: "yahoo_finance",
        ...
        console.error(
          "[market-data-import-yahoo] failed to append failure activity log",
          activityError,
        );
      }
    }
