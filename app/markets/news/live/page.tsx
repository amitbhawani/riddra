...
import {
  buildRiddraDailyMarketBriefFromArticles,
...
function buildMarketNewsPageHref(input: {
  ...
  return query ? `/markets/news/live?${query}` : "/markets/news/live";
}
...
export default async function LiveMarketNewsPage({ searchParams }: PageProps) {
  ...
  const defaultFilterOptions: Awaited<ReturnType<typeof getMarketNewsFilterOptions>> = {
    companies: [],
    sectors: [],
    categories: [],
    impactLabels: [],
  };
  let filterOptions: Awaited<ReturnType<typeof getMarketNewsFilterOptions>> =
    defaultFilterOptions;
  let dailyBrief: Awaited<ReturnType<typeof getRiddraDailyMarketBrief>> | null = null;
  let articles: Awaited<ReturnType<typeof getMarketNewsArticles>> = [];

  if (activeFilters) {
    [filterOptions, dailyBrief, articles] = await Promise.all([
      getMarketNewsFilterOptions().catch(() => defaultFilterOptions),
    ...
    articles = await getMarketNewsArticles({
      limit: UNFILTERED_MARKET_NEWS_POOL_LIMIT,
    }).catch(() => []);
    filterOptions = buildMarketNewsFilterOptionsFromArticles(articles);
    dailyBrief = buildRiddraDailyMarketBriefFromArticles(articles);
  }
        ...
        {shouldShowLatestSection ? (
          <section className="space-y-4">
            ...
            <MarketNewsList
              ...
              emptyTitle={
                activeFilters ? "No stories match these filters yet" : "Market News is being prepared"
              }
              emptyDescription={
                activeFilters
