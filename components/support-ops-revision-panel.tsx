...
type SupportOpsRevisionPanelProps = {
  items: SupportOpsItem[];
};

function getDefaultRevisionState(status: string): (typeof revisionStates)[number] {
  if (status === "In progress") return "Review ready";
  if (status === "Queued") return "Rollback staged";
  return "Published";
}

export function SupportOpsRevisionPanel({ items }: SupportOpsRevisionPanelProps) {
  const router = useRouter();
  const [selectedTitle, setSelectedTitle] = useState(items[0]?.title ?? "");
  const [editor, setEditor] = useState("Support Ops Operator");
  const selectedItem = items.find((item) => item.title === selectedTitle) ?? items[0];
  const [action, setAction] = useState(
    selectedItem ? `Logged ${selectedItem.title.toLowerCase()} mutation from support ops` : "",
  );
  const [changedFields, setChangedFields] = useState("status, queue_policy, recovery_rule");
  ...
  function syncFromSelection(title: string) {
    ...
    setAction(`Logged ${nextItem.title.toLowerCase()} mutation from support ops`);
    setChangedFields("status, queue_policy, recovery_rule");
    ...
    try {
      const response = await fetch("/api/admin/revisions", {
        ...
        body: JSON.stringify({
          ...
          assetType: "support operations lane",
          editor,
          action,
          changedFields,
          reason,
          revisionState,
          routeTarget: "/admin/support-ops",
        }),
      });

      const payload = (await response.json()) as { error?: string };
      if (!response.ok) {
        throw new Error(payload.error ?? "Unable to record support-ops revision.");
      }

      setMessage("Saved a write-through support-ops revision row.");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to record support-ops revision.");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <div className="rounded-[24px] border border-white/8 bg-black/15 p-5">
      <div className="flex flex-col gap-4">
        <div>
          <h3 className="text-lg font-semibold text-white">Write-through support action</h3>
          <p className="mt-2 text-sm leading-7 text-mist/72">
            Log support-lane changes into the shared revision lane so help, triage, and recovery operations stop living only as static readiness cards.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {pending ? "Saving…" : "Write support revision"}
          </button>
          <p className="text-xs leading-6 text-mist/60">
            {message ?? "Appends a real revision row from support ops instead of leaving support-lane changes as readiness-only copy."}
          </p>
        </div>
      </div>
    </div>
