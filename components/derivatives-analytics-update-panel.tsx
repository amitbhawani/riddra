...
type DerivativesAnalyticsOption = {
  lane: string;
...
type DerivativesAnalyticsUpdatePanelProps = {
  items: DerivativesAnalyticsOption[];
};

const statuses = ["In progress", "Planned", "Blocked"] as const;

export function DerivativesAnalyticsUpdatePanel({ items }: DerivativesAnalyticsUpdatePanelProps) {
  ...
  const [retainedSessions, setRetainedSessions] = useState(String(activeItem?.retainedSessions ?? 0));
  const [nextJob, setNextJob] = useState(activeItem?.nextJob ?? "");
  ...
  async function saveLane() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? "Unable to save derivatives analytics lane.");
      }

      setMessage("Saved derivatives analytics posture into the backend memory lane.");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to save derivatives analytics lane.");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <div className="rounded-[24px] border border-white/8 bg-black/15 p-5">
      <div className="flex flex-col gap-4">
        <div>
          <h3 className="text-lg font-semibold text-white">Write derivatives analytics update</h3>
          <p className="mt-2 text-sm leading-7 text-mist/72">
            This now writes retained-session and next-job posture into the derivatives analytics lane instead of leaving derivatives depth as read-only planning text.
          </p>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <label className="space-y-2 text-sm text-mist/78">
            ...
            <select
              ...
              onChange={(event) => {
                ...
                setRetainedSessions(String(nextItem?.retainedSessions ?? 0));
                setNextJob(nextItem?.nextJob ?? "");
          ...
          <label className="space-y-2 text-sm text-mist/78">
            ...
            <input
              value={retainedSessions}
              onChange={(event) => setRetainedSessions(event.target.value)}
              className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-white outline-none transition focus:border-white/20"
            />
          </label>
          <label className="space-y-2 text-sm text-mist/78">
            ...
            <input
              value={nextJob}
              onChange={(event) => setNextJob(event.target.value)}
              className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-white outline-none transition focus:border-white/20"
            />
          </label>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {pending ? "Saving…" : "Save analytics lane"}
          </button>
          <p className="text-xs leading-6 text-mist/60">{message ?? "Writes into the same derivatives lane used by Market Data Ops and the premium trader surfaces."}</p>
        </div>
      </div>
    </div>
  );
