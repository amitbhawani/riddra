...
type LaunchOwnerInboxRevisionPanelProps = {
  items: LaunchOwnerInboxLane[];
};

function getDefaultRevisionState(urgency: LaunchOwnerInboxLane["urgency"]): (typeof revisionStates)[number] {
  return urgency === "Critical" ? "Review ready" : "Published";
}

function getAssetLabel(item: LaunchOwnerInboxLane) {
  return `${item.owner} owner inbox`;
}

export function LaunchOwnerInboxRevisionPanel({ items }: LaunchOwnerInboxRevisionPanelProps) {
  const router = useRouter();
  const [selectedLabel, setSelectedLabel] = useState(items[0] ? getAssetLabel(items[0]) : "");
  const [editor, setEditor] = useState("Launch Owner Inbox Operator");
  const selectedItem = items.find((item) => getAssetLabel(item) === selectedLabel) ?? items[0];
  const [action, setAction] = useState(
    selectedItem ? `Logged ${selectedItem.owner.toLowerCase()} owner-inbox mutation` : "",
  );
  const [changedFields, setChangedFields] = useState("owner_lane, blocker_count, approval_count, urgency");
  ...
  const [revisionState, setRevisionState] = useState<(typeof revisionStates)[number]>(
    selectedItem ? getDefaultRevisionState(selectedItem.urgency) : "Review ready",
  );
  ...
  function syncFromSelection(label: string) {
    ...
    setAction(`Logged ${nextItem.owner.toLowerCase()} owner-inbox mutation`);
    setChangedFields("owner_lane, blocker_count, approval_count, urgency");
    setReason(nextItem.summary);
    setRevisionState(getDefaultRevisionState(nextItem.urgency));
    setMessage(null);
  }
    ...
    try {
      const response = await fetch("/api/admin/revisions", {
        ...
        body: JSON.stringify({
          ...
          assetType: "launch owner inbox lane",
          editor,
          action,
          changedFields,
          reason,
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? "Unable to record launch-owner-inbox revision.");
      }

      setMessage("Saved a write-through launch-owner-inbox revision row.");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to record launch-owner-inbox revision.");
    } finally {
      setPending(false);
    }
  }
      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-7 text-mist/72">
            Log owner-lane changes into the shared revision lane so launch owner handoffs stop living only as inbox
            summaries.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <p className="text-xs leading-6 text-mist/60">
            {message ??
              "Appends a real revision row from launch owner inbox instead of leaving owner handoff changes as static summary copy."}
          </p>
        </div>
      </div>
    </div>
