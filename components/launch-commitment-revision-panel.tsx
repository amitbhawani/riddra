...
import type { LaunchCommitmentItem } from "@/lib/launch-commitments";

const revisionStates = ["Published", "Review ready", "Rollback staged"] as const;

type LaunchCommitmentRevisionPanelProps = {
  items: LaunchCommitmentItem[];
};

function getDefaultRevisionState(status: LaunchCommitmentItem["status"]): (typeof revisionStates)[number] {
  if (status === "Ready") return "Published";
  if (status === "Deferred") return "Review ready";
  if (status === "In progress") return "Review ready";
  return "Rollback staged";
}

export function LaunchCommitmentRevisionPanel({ items }: LaunchCommitmentRevisionPanelProps) {
  const router = useRouter();
  const [selectedTitle, setSelectedTitle] = useState(items[0]?.title ?? "");
  const [editor, setEditor] = useState("Launch Commitment Operator");
  const selectedItem = items.find((item) => item.title === selectedTitle) ?? items[0];
  const [action, setAction] = useState(
    selectedItem ? `Logged ${selectedItem.title.toLowerCase()} commitment mutation from launch commitments` : "",
  );
  const [changedFields, setChangedFields] = useState("status, blocker_detail, related_surface");
  ...
  function syncFromSelection(title: string) {
    ...
    setAction(`Logged ${nextItem.title.toLowerCase()} commitment mutation from launch commitments`);
    setChangedFields("status, blocker_detail, related_surface");
    ...
    try {
      const response = await fetch("/api/admin/revisions", {
        ...
        body: JSON.stringify({
          ...
          assetType: "launch commitment lane",
          editor,
          action,
          changedFields,
          reason,
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? "Unable to record launch-commitment revision.");
      }

      setMessage("Saved a write-through launch-commitment revision row.");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to record launch-commitment revision.");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <div className="rounded-[24px] border border-white/8 bg-black/15 p-5">
      <div className="flex flex-col gap-4">
        <div>
          <h3 className="text-lg font-semibold text-white">Write-through commitment action</h3>
          <p className="mt-2 text-sm leading-7 text-mist/72">
            Log launch-commitment changes into the shared revision lane so activation blockers stop living only as a static commitment board.
          </p>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <label className="space-y-2 text-sm text-mist/78">
            <span>Commitment lane</span>
            <select
              value={selectedTitle}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {pending ? "Saving…" : "Write commitment revision"}
          </button>
          <p className="text-xs leading-6 text-mist/60">
            {message ?? "Appends a real revision row from launch commitments instead of leaving blocker changes as static board copy."}
          </p>
        </div>
      </div>
    </div>
