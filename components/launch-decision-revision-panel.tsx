...
type LaunchDecisionBlocker = {
  title: string;
  status: "Resolved" | "Blocking";
  requiredFor: "internal_review" | "launch_prep" | "private_beta" | "public_beta" | "full_launch";
  detail: string;
};

type LaunchDecisionRevisionPanelProps = {
  items: LaunchDecisionBlocker[];
};

function getDefaultRevisionState(status: LaunchDecisionBlocker["status"]): (typeof revisionStates)[number] {
  return status === "Resolved" ? "Published" : "Rollback staged";
}

export function LaunchDecisionRevisionPanel({ items }: LaunchDecisionRevisionPanelProps) {
  const router = useRouter();
  const [selectedTitle, setSelectedTitle] = useState(items[0]?.title ?? "");
  const [editor, setEditor] = useState("Launch Decision Operator");
  const selectedItem = items.find((item) => item.title === selectedTitle) ?? items[0];
  const [action, setAction] = useState(
    selectedItem ? `Logged ${selectedItem.title.toLowerCase()} launch-decision mutation` : "",
  );
  const [changedFields, setChangedFields] = useState("decision_state, required_launch_mode, blocker_detail");
  ...
  const [revisionState, setRevisionState] = useState<(typeof revisionStates)[number]>(
    selectedItem ? getDefaultRevisionState(selectedItem.status) : "Rollback staged",
  );
  ...
  function syncFromSelection(title: string) {
    ...
    setAction(`Logged ${nextItem.title.toLowerCase()} launch-decision mutation`);
    setChangedFields("decision_state, required_launch_mode, blocker_detail");
    ...
    try {
      const response = await fetch("/api/admin/revisions", {
        ...
        body: JSON.stringify({
          ...
          assetType: "launch decision blocker",
          editor,
          action,
          changedFields,
          reason,
          revisionState,
          routeTarget: "/admin/launch-decision",
        }),
      });

      const payload = (await response.json()) as { error?: string };
      if (!response.ok) {
        throw new Error(payload.error ?? "Unable to record launch-decision revision.");
      }

      setMessage("Saved a write-through launch-decision revision row.");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to record launch-decision revision.");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <div className="rounded-[24px] border border-white/8 bg-black/15 p-5">
      <div className="flex flex-col gap-4">
        <div>
          <h3 className="text-lg font-semibold text-white">Write-through decision action</h3>
          <p className="mt-2 text-sm leading-7 text-mist/72">
            Log launch-decision changes into the shared revision lane so the current go-live judgment stops living only as a summary board.
          </p>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <label className="space-y-2 text-sm text-mist/78">
            <span>Decision blocker</span>
            <select
              value={selectedTitle}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {pending ? "Saving…" : "Write decision revision"}
          </button>
          <p className="text-xs leading-6 text-mist/60">
            {message ?? "Appends a real revision row from launch decision instead of leaving go-live judgment changes as a summary-only board."}
          </p>
        </div>
      </div>
    </div>
