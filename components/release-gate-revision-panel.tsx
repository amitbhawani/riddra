...
type ReleaseGateRevisionPanelProps = {
  items: ReleaseGateItem[];
};

export function ReleaseGateRevisionPanel({ items }: ReleaseGateRevisionPanelProps) {
  const router = useRouter();
  const [selectedTitle, setSelectedTitle] = useState(items[0]?.title ?? "");
  const [editor, setEditor] = useState("Release Gate Operator");
  const selectedItem = items.find((item) => item.title === selectedTitle) ?? items[0];
  const [action, setAction] = useState(
    selectedItem ? `Logged ${selectedItem.title.toLowerCase()} release-gate mutation` : "",
  );
  const [changedFields, setChangedFields] = useState("gate_summary, launch_gate_metric, release_truth");
  ...
  function syncFromSelection(title: string) {
    ...
    setAction(`Logged ${nextItem.title.toLowerCase()} release-gate mutation`);
    setChangedFields("gate_summary, launch_gate_metric, release_truth");
    ...
    try {
      const response = await fetch("/api/admin/revisions", {
        ...
        body: JSON.stringify({
          ...
          assetType: "release gate lane",
          editor,
          action,
          changedFields,
          reason,
          revisionState,
          routeTarget: "/admin/release-gate-board",
        }),
      });

      const payload = (await response.json()) as { error?: string };
      if (!response.ok) {
        throw new Error(payload.error ?? "Unable to record release-gate revision.");
      }

      setMessage("Saved a write-through release-gate revision row.");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to record release-gate revision.");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <div className="rounded-[24px] border border-white/8 bg-black/15 p-5">
      <div className="flex flex-col gap-4">
        <div>
          <h3 className="text-lg font-semibold text-white">Write-through release-gate action</h3>
          <p className="mt-2 text-sm leading-7 text-mist/72">
            Log release-gate changes into the shared revision lane so final gate shifts stop living only as a summary board.
          </p>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <label className="space-y-2 text-sm text-mist/78">
            ...
            <input
              value="/admin/release-gate-board"
              readOnly
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {pending ? "Saving…" : "Write gate revision"}
          </button>
          <p className="text-xs leading-6 text-mist/60">
            {message ?? "Appends a real revision row from release-gate-board instead of leaving final gate changes as static board summaries."}
          </p>
        </div>
      </div>
    </div>
